...
def _filter_to_certain_chapters(df, book_chapter: pd.DataFrame):
    ...
    logging.info(f"Reporting on the reduced list of {len(lo_chapters)} chapters")
...
def report_table_full_known(sim_res_full, report_params,
                            known_authors, chapters_to_report):
    if not chapters_to_report.empty:
    ...
    sig_level = report_params['sig_level']
